		  ...
		  <nav class=" navbar navbar-expand-md navbar-dark fixed-top navbar-transparente d-flex">
		     ...
		     <div class="collapse navbar-collapse" id="navegacao">
		        <ul class="navbar-nav ">
                    ...
                    <li class="nav-item" >
		              <a href="menu_inicial_agendamento_profissional.php" class="nav-link active" >Agenda</a>
		            </li>
		            <li class="nav-item">
      ...
      <div class="col-12"> 
        ...
        <div class="row  justify-content-md-center">
            <div class="col-10 caixa-funcionamento-listar">
                   ...
                   $sqlServicoNome = "SELECT * FROM `agendamento` WHERE id_Salao = '" . $_SESSION["idSalao"] . "'";
                   $conServicoNome = $conn->query($sqlServicoNome);
                   $qtdServicoNome = $conServicoNome-> num_rows;
                   ...
                   if($qtdServicoNome > 0){
                    ...
                    print "<th>Profissional responsavel</th>";
                    print "<th>Cliente</th>";
                    print "<th>Horario agendado</th>";
                    print "<th>Dia agendado</th>";
                    print "<tr>";
                    while($row = $conServicoNome->fetch_object()){
                    $dataFormatada= date("d/m/Y", strtotime($row->DataAgendamento));  

                    $sqlProfissional = "SELECT NomeProfissional FROM `profissional` WHERE id_Profissional = '{$row->id_Profissional}' ";
                    $conProfissional = $conn->query($sqlProfissional) or die($mysqli->error);
                    $qtdProfissional = $conProfissional->fetch_array(); 
                    
                    
                    $sqlCliente = "SELECT NomeCliente FROM `cliente` WHERE id_Cliente = '{$row->id_cliente}' ";
                    $conCliente = $conn->query($sqlCliente) or die($mysqli->error);
                    $qtdCliente = $conCliente->fetch_array();

                    print "<tr>";
                    print "<td>". $row->TipoServico; "</td>";
                    print "<td>". $row->Servico; "</td>";
                    print "<td>". $row->ValorServico; "</td>";
                    print "<td>". $qtdProfissional["NomeProfissional"];"</td>" ;
                    print "<td>". $qtdCliente["NomeCliente"] ;"</td>";
                    print "<td>". $dataFormatada ;"</td>";
                    print "<td>". $row->HoraAgendamento ;"</td>";
